...
import com.amazon.aace.AlexaClient;
import com.amazon.aace.AlexaProperties;
import com.amazon.aace.CoreProperties;
import com.amazon.aace.Engine;
import com.amazon.aace.NavigationProperties;
import com.amazon.aace.config.AlexaConfiguration;
import com.amazon.aace.config.ConfigurationFile;
import com.amazon.aace.config.EngineConfiguration;
import com.amazon.aace.config.StorageConfiguration;
import com.amazon.aace.config.VehicleConfiguration;
...
public class AsvAlexaPlugin implements Observer {
    ...
    private void create() {
        ...
        mAudioCueEnd = MediaPlayer.create(mContext, R.raw.med_ui_endpointing_touch);

        // Get shared preferences
        mPreferences = mContext.getSharedPreferences(mContext.getString(R.string.preference_file_key),
    ...
    private void copyAsset(String assetPath, File destFile, boolean force) {
        FileUtils.copyAsset(mActivity.getAssets(), assetPath, destFile, force);
    }
    ...
    private void startEngine(String json) throws RuntimeException {
        ...
        // AlexaSpeaker
        if (!mEngine.registerPlatformInterface(
                mAlexaSpeaker = new AlexaSpeakerHandler(mActivity, mLogger)
        )
        ...
        // NetworkInfoProvider
        if (!mEngine.registerPlatformInterface(
                mNetworkInfoProvider = new NetworkInfoProviderHandler(mActivity, mLogger, mEngine)
        )
        ) throw new RuntimeException("Could not register NetworkInfoProvider platform interface");

        // CBL Auth Handler
        LoginWithAmazonCBL LoginHandler = new LoginWithAmazonCBL(mActivity, mLogger);

        // AuthProvider
        if (!mEngine.registerPlatformInterface(
                mAuthProvider = new AuthProviderHandler(mActivity, mLogger, LoginHandler)
        )
        ...
        // Navigation
        if (!mEngine.registerPlatformInterface(
                mNavigation = new NavigationHandler(mActivity, mLogger)
        )
        ) throw new RuntimeException("Could not register Navigation platform interface");
        else
            mEngine.setProperty(NavigationProperties.NAVIGATION_PROVIDER_NAME, "HERE"); //set default provider name
        ...
        if (!mEngine.registerPlatformInterface(
                mAddressBook = new AddressBookHandler(mActivity, mLogger, sampleContactsDataPath, sampleNavigationFavoritesDataPath)
        )
        ) throw new RuntimeException("Could not register AddressBook platform interface");

        // EqualizerController
        if (!mEngine.registerPlatformInterface(
                mEqualizerControllerHandler = new EqualizerControllerHandler(mActivity, mLogger)
        )
        ...
        mMACCPlayer = new MACCPlayer(mActivity, mLogger, mPlaybackController);
        ...
        if (!mEngine.registerPlatformInterface(
                mCDLocalMediaSource = new CDLocalMediaSource(mContext, mLogger)
        ))
        ...
        if (!mEngine.registerPlatformInterface(
                mDABLocalMediaSource = new DABLocalMediaSource(mContext, mLogger)
        ))
        ...
        if (!mEngine.registerPlatformInterface(
                mAMLocalMediaSource = new AMLocalMediaSource(mContext, mLogger)
        ))
        ...
        if (!mEngine.registerPlatformInterface(
                mSIRUSXMLocalMediaSource = new SiriusXMLocalMediaSource(mContext, mLogger)
        ))
        ...
        if (!mEngine.registerPlatformInterface(
                mFMLocalMediaSource = new FMLocalMediaSource(mContext, mLogger)
        ))
        ...
        if (!mEngine.registerPlatformInterface(
                mBTLocalMediaSource = new BluetoothLocalMediaSource(mContext, mLogger)
        ))
        ...
        if (!mEngine.registerPlatformInterface(
                mLILocalMediaSource = new LineInLocalMediaSource(mContext, mLogger)
        ))
        ...
        if (!mEngine.registerPlatformInterface(
                mSATRADLocalMediaSource = new SatelliteLocalMediaSource(mContext, mLogger)
        ))
        ...
        if (!mEngine.registerPlatformInterface(
                mUSBLocalMediaSource = new USBLocalMediaSource(mContext, mLogger)
        ))
            throw new RuntimeException("Could not register Mock USB player Local Media Source platform interface");

        // Mock global preset
        if (!mEngine.registerPlatformInterface(
                mGlobalPresetHandler = new GlobalPresetHandler(mContext, mLogger)
    ...
    public String tapToTalk() {
        ...
        if (mAlexaClient != null && mSpeechRecognizer != null) {
            if (mAlexaClient.getConnectionStatus()
                ...
                res = "AlexaClient not connected. ConnectionStatus: "
                        + mAlexaClient.getConnectionStatus();
            }
        } else {
            res = "!mAlexaClient or !mSpeechRecognizer";
        }
